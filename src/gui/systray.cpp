...
{
    const auto iconRect = geometry();

    if (iconRect.isNull()) {
        // QSystemTrayIcon::geometry() is broken for ages on most Linux DEs, thus
        // fall back to mouse position (assuming tray icon is activated by click)
        return QCursor::pos();
    }
